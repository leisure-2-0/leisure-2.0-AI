...
// 어떤 필드를 채워서 넘길지(예: 축제 payload엔 status 필드가 아예 없으니 축제 검색이면 null로 둘 것)는
// CollectionRouter 책임 - 여기는 받은 값을 그대로 믿고 변환만 함.
@Component
public class SearchFilterMapper {
    ...
    public Filter.Expression toExpression(SearchFilter filter) {
        ...
        if (filter.status() != null) {combined = and(combined, builder.eq("status", filter.status()));}
